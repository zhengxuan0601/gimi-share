<template>
  <div id="circle-comments" class="circle-comments">
        ...
        <div class="right">
          ...
          <div class="bottom-operate">
            ...
            <a-button
              :disabled="!comment"
              type="primary" 
              @click="submitComment(comment, circleDetail.userId, {})">评论</a-button>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="right">
            <div class="user">
              <nuxt-link 
                ...
                <span 
                  v-if="item.userId === circleDetail.userId" 
            ...
            <div class="comment">
              <p>{{ item.content }}</p>
              <span 
                v-if="item.isAuthor || userInfo.id === circleDetail.userId" 
                ...
                <div class="right">
                  <div class="user">
                    <nuxt-link
                      ...
                      <span 
                        v-if="itemName.userId === circleDetail.userId" 
                        class="is-author">(作者)</span>
                    </nuxt-link>
                  ...
                  <div class="comment">
                    ...
                    <span 
                      v-if="itemName.isAuthor || userInfo.id === circleDetail.userId" 
  ...
  methods: {
    ...
    async submitComment (comment, uid, { 
      ...
      try {
        ...
        await this.$axios.post('/api/v1/sharecircle/comments/create', commentInfo)
        this.findComments()
    ...
    async findComments () {
      try {
        const { data } = await this.$axios.get(`/api/v1/sharecircle/comments?circleId=${this.circleDetail.id}`)
    ...
    async isLikeComment (commentItem) {
      ...
      try {
        ...
        await this.$axios.get(`/api/v1${API}?commentId=${commentItem.id}&itemType=2`)
    ...
    async deleteComment (commentId) {
      try {
        await this.$axios.get(`/api/v1/sharecircle/comments/delete?id=${commentId}`)
        this.findComments()
      } catch (error) {
        console.log(error)
      }
